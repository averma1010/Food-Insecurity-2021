...
levels(FS_Subset$SNAP) <- c('Refused', 'Dont Know', 'Uses SNAP', 'No SNAP')
...
* Just adding all the variables in the in the logistic regression to see what happens.We can then decide to take some 

```{r, results='markup'}
logistic <- glm(FS_Status ~ States + Family_Size + Household_Income + SNAP + Citizenship_status + Number_of_Jobs + Hours_on_Jobs + Education_Level + Number_of_children + Ethnicity , data = train, family = "binomial")
