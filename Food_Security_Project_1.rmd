...
selected <- c('White only', 'Black only', 'American Indian, Alaskan native only', 'Asian Only', 'Hawaiian', 'White-black', 'White-AI', 'White-Asian', 'White-HP', 'Black-AI', 'Black-Asian', 'Black-HP', 'AI-Asian', 'AI-HP', 'Asian-HP', 'W-B-AI', 'W-B-A', 'W-B-HP', 'W-AI-A', 'W-AI-HP', 'W-A-HP', 'B-AI-A', 'W-B-AL-A', 'W-AI-A-HP')
...
chi_test <- table(FS_ss$Ethnicity, FS_ss$FoodSecurity_score)
chi_test
str(FS_ss)
chisq.test(chi_test)
```
...
childless_fam<-subset(FS_Subset, PRNMCHLD==0, select = c(HRHHID, PRNMCHLD))
...
food_una <- subset(FS_Subset, select = c("Id", "States","Family_Size","Household_Income","FoodSecurity_score", "Number_of_Children"))
head(food_una)
...
```{r data_una, results='markup'}
ok_state<-subset(FS_Subset, States=="40")
me_state<-subset(FS_Subset, States=="23")
table(ok_state$Household_Income)
sum(table(ok_state$Household_Income))
table(me_state$Household_Income)
sum(table(me_state$Household_Income))
```
...
qqplot(food_una$Household_Income, food_una$FoodSecurity_score, plot.it = TRUE,  xlab="Household income", ylab="Food security Score", main="QQ Plot for Food security vs Household income")
...
qqplot(food_una$Family_Size, food_una$FoodSecurity_score, plot.it = TRUE,  xlab="Family size", ylab="Food security Score", main="QQ Plot for Food security vs Family")
