...
levels(FS_Subset$FS_Status) <- c("No respone", "Food Secure", "Food Secure", "Food Insecure", "Food Insecure")
...
ggplot(FS_Subset, aes(x=FoodSecurity_score, fill=FoodSecurity_score)) + geom_bar( aes(y = (..count..)/sum(..count..)*100), position = "dodge") 
...
ggplot(pie_FS, aes(x = "", y = perc, fill = FS_Status)) +
  geom_col() +
...
for(i in levels(FS_Subset$Ethnicity)){
  ...
  print(ggplot(subset(FS_Subset, Ethnicity == i ) , aes(x= Ethnicity, color =FS_Status, fill = FS_Status)) + geom_bar( aes(y = (..count..)/sum(..count..)*100), position = "dodge"))
}
```
```{r, results='markup'}
